...
def todo(request):
    ...
    if request.method == "POST":  # проверяем то что метод именно POST
        ...
        if "Delete" in request.POST:  # если пользователь собирается удалить одно дело
            checkedlist = request.POST.getlist('checkedbox')  # берем список выделенные дел,
            # которые мы собираемся удалить
...
def category(request):
    ...
    if request.method == "POST":  # проверяем что это метод POST
        ...
        if "Delete" in request.POST:  # проверяем есть ли удаление
            check = request.POST.getlist('check')  # немного изменил название массива в отличии
            # от todo, что бы было меньше путаницы в коде
